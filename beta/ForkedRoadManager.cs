...
internal static partial class ForkedRoadManager
{
    ...
    private static SpectatorCombatRenderState? _renderedSpectatorCombatView;
    ...
    private static readonly System.Reflection.MethodInfo? RunManagerAfterMapLocationChangedMethod =
        AccessTools.Method(typeof(RunManager), "AfterMapLocationChanged");
    ...
    private static readonly int?[] BufferedRunLocationRoomIds = new int?[] { null, 0, 1 };
    ...
    private sealed class SpectatorCombatRenderState
    {
        ...
        public required Dictionary<ulong, Player> RoomPlayersByOriginalId { get; init; }

        public required List<Creature> NonPlayerAllies { get; init; }

        public required List<Creature> EnemyCreatures { get; init; }
    }
    ...
    private static IEnumerable<RunLocation> EnumerateBufferedRunLocationVariants(MapLocation mapLocation)
    {
        foreach (int? roomId in BufferedRunLocationRoomIds)
        {
            yield return new RunLocation(mapLocation, roomId);
        }
    }

    private static void AddBufferedRunLocationVariants(ISet<RunLocation> visitedLocations, MapLocation mapLocation, ISet<RunLocation>? seededLocations = null)
    {
        foreach (RunLocation location in EnumerateBufferedRunLocationVariants(mapLocation))
        {
            visitedLocations.Add(location);
            seededLocations?.Add(location);
        }
    }
    ...
    private static void EnsureCurrentRunLocationVisited()
    {
        ...
        HashSet<RunLocation> visitedLocations = RunLocationVisitedLocationsRef(RunManager.Instance.RunLocationTargetedBuffer);
        visitedLocations.Add(_runState.RunLocation);
    }
    ...
    internal static void CleanUp()
    {
        ...
        _renderedSpectatorCombatView = null;
    ...
    private static void RefreshSpectatorViewContent(PlayerBranchRuntime player, SpectatorRuntimeState? spectatorState)
    {
        ...
        {
            bool isSameCombatView = _renderedSpectatorCombatView != null &&
                                    _renderedSpectatorBranchId == branch.BranchId &&
                                    _renderedSpectatorPlayerId == viewedPlayerId;
            if (isSameCombatView)
            {
                if (_renderedSpectatorRevision == revision)
                {
                    return;
                }

                if (TryUpdateSpectatorCombatRenderState(_renderedSpectatorCombatView!, branch, viewedPlayerId))
                {
            ...
            _renderedSpectatorPlayerId = viewedPlayerId;
            _renderedSpectatorRevision = revision;
            _renderedSpectatorCombatView = CreateSpectatorCombatRenderState(branch, viewedPlayerId);
            _spectatorOverlay.SetViewContent(_renderedSpectatorCombatView?.Root);
            return;
        }
    ...
    private static Control? CreateSpectatorFullScreenView(BranchGroupRuntime branch, ulong? viewedPlayerId)
    {
        ...
        if (branch.LatestCombatState != null && branch.EncounterId != null)
        {
            return CreateSpectatorCombatRenderState(branch, viewedPlayerId)?.Root;
        }
    ...
    private static SpectatorCombatRenderState? CreateSpectatorCombatRenderState(BranchGroupRuntime branch, ulong? viewedPlayerId)
    {
        ...
        try
        {
            ...
            List<Creature> nonPlayerAllies = new();
            ...
            void ConfigureRoom()
            {
                ...
                SyncSpectatorCombatUi(room, uiPlayer);
            ...
            return new SpectatorCombatRenderState
            {
                ...
                RoomPlayersByOriginalId = roomPlayersByOriginalId,
                NonPlayerAllies = nonPlayerAllies,
                EnemyCreatures = combatState.Enemies.ToList()
            };
        }
    ...
    private static bool TryUpdateSpectatorCombatRenderState(SpectatorCombatRenderState renderState, BranchGroupRuntime branch, ulong? viewedPlayerId)
    {
        if (_runState == null || branch.LatestCombatState == null || branch.EncounterId == null)
        {
            return false;
        }

        NetFullCombatState snapshot = branch.LatestCombatState;
        if (!CanReuseSpectatorCombatRenderState(renderState, branch, snapshot))
        {
            return false;
        }
        ...
        foreach (NetFullCombatState.PlayerState playerState in snapshot.Players)
        {
            if (!renderState.RoomPlayersByOriginalId.TryGetValue(playerState.playerId, out Player? roomPlayer) ||
                !allyCreatureStates.TryGetValue(playerState.playerId, out NetFullCombatState.CreatureState creatureState))
            {
                return false;
            }

            ApplySpectatorCreatureSnapshot(roomPlayer.Creature, creatureState);
            ApplySpectatorPlayerCombatSnapshotInPlace(roomPlayer, playerState);
        }

        IReadOnlyList<NetFullCombatState.CreatureState> creatures = snapshot.Creatures;
        int alliedCount = System.Math.Clamp(branch.AlliedCreatureCount, 0, creatures.Count);
        List<NetFullCombatState.CreatureState> nonPlayerAllyStates = creatures
            .Take(alliedCount)
            .Where(static state => !state.playerId.HasValue && state.monsterId != null)
            .ToList();
        for (int i = 0; i < nonPlayerAllyStates.Count; i++)
        {
            ApplySpectatorCreatureSnapshot(renderState.NonPlayerAllies[i], nonPlayerAllyStates[i]);
        }

        List<NetFullCombatState.CreatureState> enemyStates = creatures
            .Skip(alliedCount)
            .Where(static state => state.monsterId != null)
            .ToList();
        for (int i = 0; i < enemyStates.Count; i++)
        {
            ApplySpectatorCreatureSnapshot(renderState.EnemyCreatures[i], enemyStates[i]);
        }

        NetFullCombatState.PlayerState? primaryState = ResolvePrimarySpectatorPlayerState(branch, snapshot, viewedPlayerId);
        if (primaryState == null || !renderState.RoomPlayersByOriginalId.TryGetValue(primaryState.Value.playerId, out Player? uiPlayer))
        {
            return false;
        }

        SyncSpectatorCombatUi(renderState.Room, uiPlayer);
        Log.Info($"ForkedRoad refreshed spectator combat room in place: branch={branch.BranchId} uiPlayer={uiPlayer.NetId} hand={(uiPlayer.PlayerCombatState?.Hand.Cards.Count ?? 0)} play={(uiPlayer.PlayerCombatState?.PlayPile.Cards.Count ?? 0)} creatures={renderState.CombatState.Creatures.Count}.");
        return true;
    }

    private static bool CanReuseSpectatorCombatRenderState(SpectatorCombatRenderState renderState, BranchGroupRuntime branch, NetFullCombatState snapshot)
    {
        if (branch.EncounterId == null || renderState.BranchId != branch.BranchId || renderState.EncounterId != branch.EncounterId)
        {
            return false;
        }

        List<ulong> snapshotPlayerIds = snapshot.Players.Select(static player => player.playerId).ToList();
        if (renderState.RoomPlayersByOriginalId.Count != snapshotPlayerIds.Count ||
            snapshotPlayerIds.Any(playerId => !renderState.RoomPlayersByOriginalId.ContainsKey(playerId)))
        {
            return false;
        }
        ...
        List<NetFullCombatState.CreatureState> nonPlayerAllyStates = creatures
            ...
            .Where(static state => !state.playerId.HasValue && state.monsterId != null)
            .ToList();
        if (renderState.NonPlayerAllies.Count != nonPlayerAllyStates.Count)
        {
            return false;
        }
        ...
        for (int i = 0; i < nonPlayerAllyStates.Count; i++)
        {
            if (renderState.NonPlayerAllies[i].Monster?.Id != nonPlayerAllyStates[i].monsterId)
            {
                return false;
            }
        }
        ...
        List<NetFullCombatState.CreatureState> enemyStates = creatures
            ...
            .Where(static state => state.monsterId != null)
            .ToList();
        if (renderState.EnemyCreatures.Count != enemyStates.Count)
        {
            return false;
        }
        ...
        for (int i = 0; i < enemyStates.Count; i++)
        {
            if (renderState.EnemyCreatures[i].Monster?.Id != enemyStates[i].monsterId)
            {
                return false;
            }
        }
    ...
    private static void ApplySpectatorPlayerCombatSnapshotInPlace(Player player, NetFullCombatState.PlayerState playerState)
    {
        player.Gold = playerState.gold;
        if (player.PlayerCombatState == null)
        {
        ...
        foreach (NetFullCombatState.CombatPileState pileState in playerState.piles)
        {
            ...
            SyncSpectatorCardPile(pile, pileState);
        }
    }

    private static void SyncSpectatorCardPile(CardPile pile, NetFullCombatState.CombatPileState pileState)
    {
        pile.Clear(silent: true);
        foreach (NetFullCombatState.CardState cardState in pileState.cards)
        {
    ...
    private static void SyncSpectatorCombatUi(NCombatRoom room, Player uiPlayer)
    {
        if (_runState != null && room.Background == null)
        {
            room.SetUpBackground(_runState);
        }

        room.SetWaitingForOtherPlayersOverlayVisible(false);
        SyncSpectatorHandUi(room.Ui.Hand, uiPlayer);
        SyncSpectatorPlayPileUi(room.Ui, uiPlayer);
        RefreshSpectatorCombatRoomLayout(room);
    }

    private static void SyncSpectatorHandUi(NPlayerHand hand, Player uiPlayer)
    {
        if (uiPlayer.PlayerCombatState == null)
        {
            return;
        ...
        IReadOnlyList<CardModel> cards = uiPlayer.PlayerCombatState.Hand.Cards;
        List<NHandCardHolder> holders = hand.CardHolderContainer.GetChildren().OfType<NHandCardHolder>().ToList();
        while (holders.Count > cards.Count)
        {
            NHandCardHolder holder = holders[^1];
            holders.RemoveAt(holders.Count - 1);
            hand.RemoveCardHolder(holder);
        }

        HashSet<NHandCardHolder> newlyAdded = new();
        for (int i = 0; i < cards.Count; i++)
        {
            CardModel card = cards[i];
            if (i < holders.Count)
            {
                holders[i].ReassignToCard(card, PileType.Hand, null, ModelVisibility.Visible);
            }
            else
            {
                NCard? cardNode = NCard.Create(card);
                ...
                NHandCardHolder holder = hand.Add(cardNode, i);
                holders.Add(holder);
                newlyAdded.Add(holder);
            }
        }

        hand.ForceRefreshCardIndices();
        Vector2 handScale = HandPosHelper.GetScale(cards.Count == 0 ? 1 : cards.Count);
        for (int i = 0; i < holders.Count; i++)
        {
            ...
            if (newlyAdded.Contains(holder))
            {
                Vector2 targetPosition = HandPosHelper.GetPosition(cards.Count, i);
                holder.Position = targetPosition;
                holder.SetTargetPosition(targetPosition);
                holder.SetAngleInstantly(HandPosHelper.GetAngle(cards.Count, i));
                holder.SetTargetAngle(HandPosHelper.GetAngle(cards.Count, i));
                holder.SetScaleInstantly(handScale);
                holder.SetTargetScale(handScale);
                holder.Hitbox.SetEnabled(enabled: true);
            }
        }

        ForkedRoadSpectatorOverlay.ApplyReadOnlyRecursive(hand);
    }

    private static void SyncSpectatorPlayPileUi(NCombatUi ui, Player uiPlayer)
    {
        if (uiPlayer.PlayerCombatState == null)
        {
            return;
        ...
        IReadOnlyList<CardModel> playCards = uiPlayer.PlayerCombatState.PlayPile.Cards;
        List<NCard> existingCards = ui.PlayContainer.GetChildren().OfType<NCard>().ToList();
        while (existingCards.Count > playCards.Count)
        {
            NCard cardNode = existingCards[^1];
            existingCards.RemoveAt(existingCards.Count - 1);
            ui.PlayContainer.RemoveChild(cardNode);
            cardNode.QueueFree();
        }

        for (int i = 0; i < playCards.Count; i++)
        {
            CardModel card = playCards[i];
            NCard? cardNode;
            if (i < existingCards.Count)
            {
                cardNode = existingCards[i];
                cardNode.Model = card;
            }
            else
            {
            ...
            cardNode.Position = PileType.Play.GetTargetPosition(cardNode) + Vector2.Left * 36f * i;
            ForkedRoadSpectatorOverlay.ApplyReadOnlyRecursive(cardNode);
        }
    }
    ...
    private static Control? CreateSpectatorRestSiteView(BranchGroupRuntime branch, ulong? viewedPlayerId)
    {
        ...
        try
        {
            ...
            RunState visualRunState = RunState.CreateForTest(
                visualPlayers,
                _runState.Acts.Select(act => act.ToMutable()).ToList(),
                _runState.Modifiers.ToList(),
                _runState.GameMode,
                _runState.AscensionLevel,
    ...
    private static void SeedBranchRunLocations(BranchBatchRuntime batch)
    {
        ...
        foreach (BranchGroupRuntime branch in batch.BranchGroups)
        {
            ...
            AddBufferedRunLocationVariants(visitedLocations, mapLocation, SeededSplitLocations);
        }
    }
    ...
    {
        return ShouldIgnoreForeignRoomScopedLocation(location);
    }
    ...
    internal static bool ShouldIgnoreBufferedAction(ActionEnqueuedMessage message)
    {
        ...
        return ShouldIgnoreForeignRoomScopedLocation(message.location);
    }
    ...
    private static bool MatchesCurrentRunLocation(RunLocation location)
    {
        if (_runState == null)
        {
            return true;
    ...
    private static bool MatchesCurrentMapLocation(RunLocation location)
    {
        if (_runState == null)
        {
            return true;
        ...
        return location.mapLocation == _runState.MapLocation;
    }
    ...
    private static void SeedMapVoteLocationsIfNeeded()
    {
        ...
        foreach (PlayerBranchRuntime player in Runtime.Players.Values)
        {
            ...
            MapLocation mapLocation = new(player.SelectionCoord.Value, _runState!.CurrentActIndex);
            AddBufferedRunLocationVariants(visitedLocations, mapLocation, SeededMapVoteLocations);
        }
    }
    ...
    internal static bool ShouldInterceptForeignMapVoteRequest(RequestEnqueueActionMessage message)
    {
        ...
        return !MatchesCurrentMapLocation(message.location);
    }
    ...
    private static bool ShouldIgnoreForeignRoomScopedLocation(RunLocation location)
    {
        if (_runState == null)
        {
            return false;
        ...
        return !MatchesCurrentRunLocation(location);
    }
    ...
    private static void NotifyRunLocationChangedAfterDebugEntry()
    {
        ...
        if (RunManagerAfterMapLocationChangedMethod != null)
        {
            RunManagerAfterMapLocationChangedMethod.Invoke(RunManager.Instance, Array.Empty<object>());
            return;
        }

        RunManager.Instance.MapSelectionSynchronizer.OnLocationChanged(_runState.MapLocation);
        RunManager.Instance.RunLocationTargetedBuffer.OnLocationChanged(_runState.RunLocation);
    }
